---
layout: page
title: Links
comments: false
---

### Blogging on Jekyll

* [GitHub Pages](https://pages.github.com/)
* [Mediumish](https://jekyllthemes.io/theme/mediumish)
* [Jekyll Themes](https://jekyllthemes.io)
* [Jekyll Installation Instructions](https://jekyllrb.com/docs/installation/macos/)
* [Creating and Hosting a Personal Site on GitHub](http://jmcglone.com/guides/github-pages/)

### Markdown
* [Markdown Cheatsheet](https://github.com/adam-p/markdown-here/wiki/Markdown-Cheatsheet)
* [Kramdown Markdown Reference](https://kramdown.gettalong.org/quickref.html)


### Photos

* [Free Photos from Unsplash](https://unsplash.com/)

### Application Development Reference

* [Patterns of Enterprise Application Architecture](https://martinfowler.com/books/eaa.html)
* [The different types of software testing](https://www.atlassian.com/continuous-delivery/software-testing/types-of-software-testing)
* [12-Factor Application](https://12factor.net/)



### Node.js

* [Awesome Node.js](https://github.com/sindresorhus/awesome-nodejs)

### Utilities
* [`nodemon`](https://github.com/remy/nodemon)
* [Yeoman](https://yeoman.io)

#### Libraries

##### Express and Express Middleware
* [Express](https://expressjs.com/)
* [Express middleware](https://expressjs.com/en/resources/middleware.html)
* [`morgan`](https://www.npmjs.com/package/morgan)
* [`morgan` Docs](https://github.com/expressjs/morgan)
* [`multer`](https://www.npmjs.com/package/multer)
* [`multer` Docs](https://github.com/expressjs/multer)
* [`connect-rid`](https://www.npmjs.com/package/connect-rid)
* [`serve-static`](https://www.npmjs.com/package/serve-static)
* [`serve-favicon`](https://www.npmjs.com/package/serve-favicon)

##### Utilities
* [`Lodash`](https://www.npmjs.com/package/lodash)
* [`Lodash` Docs](https://lodash.com/)
* [`Moment.js`](https://www.npmjs.com/package/moment)
* [`Moment.js`Docs](http://momentjs.com/)
* [bluebird](https://www.npmjs.com/package/bluebird)
* [SuperAgent](https://www.npmjs.com/package/superagent)
* [SuperAgent Docs](https://visionmedia.github.io/superagent/)
* [Mongoose](https://www.npmjs.com/package/mongoose)
* [Mongoose Docs](https://mongoosejs.com/)
* [Sequelize](https://www.npmjs.com/package/sequelize)
* [Passport](https://www.npmjs.com/package/passport)
* [Passport Docs](http://www.passportjs.org/)
* [Winston](https://www.npmjs.com/package/winston)
* [Winston Docs](https://github.com/winstonjs/winston)
* [Cheerio](https://www.npmjs.com/package/cheerio)
* [Cheerio Docs](https://github.com/cheeriojs/cheerio)
* [Forge](https://www.npmjs.com/package/node-forge)
* [Forge Docs](https://github.com/digitalbazaar/forge)
* [Commander](https://www.npmjs.com/package/commander)
* [Commander Docs](https://tj.github.io/commander.js/)
* [Telegraf](https://www.npmjs.com/package/telegraf)
* [Telegraf Docs](https://github.com/telegraf/telegraf)

#### Testing
* [`mocha`](https://mochajs.org/)
* [`chai`](https://www.chaijs.com/)
* [`chai` expect syntax](https://www.chaijs.com/guide/styles/#expect)
* [`chai-as-promised`](https://github.com/domenic/chai-as-promised)
* [`supertest`](https://github.com/visionmedia/supertest)


### Docker
* [Dockerizing a Node.js web app](https://nodejs.org/en/docs/guides/nodejs-docker-webapp/)


### Software


#### Virtual Machine Hypervisors
* [Oracle VirtualBox](https://www.virtualbox.org/wiki/Downloads)
* [VMWare Workstation](https://www.vmware.com/products/workstation-pro/workstation-pro-evaluation.html)

#### Linux
* [CentOS](https://www.centos.org/download/)
* [Workstation](https://getfedora.org/en/workstation/download/)
* [Server](https://getfedora.org/en/server/download/)
* [Desktop](https://www.ubuntu.com/download/desktop)
* [Server](https://www.ubuntu.com/download/server)

#### Integrated Development Environments
* [JetBrains Toolbox](https://www.jetbrains.com/toolbox/)
* [Visual Studio Code](https://code.visualstudio.com/)
* [Sublime Text 3](https://www.sublimetext.com/3)


#### Browsers
* [Google Chrome](https://www.google.com/chrome/)
* [Mozilla Firefox](https://www.mozilla.org/en-US/firefox/)


#### Programming Languages

##### Node.js
* [Node.js](https://nodejs.org/en/)
* [Node Version Manager](https://github.com/creationix/nvm)


##### Python
* [Python](https://www.python.org/downloads/)
* [`pyenv`](https://github.com/pyenv/pyenv)
* [`pyenv-virtualenv`](https://github.com/pyenv/pyenv-virtualenv)
* [`pyenv-installer`](https://github.com/pyenv/pyenv-installer)


##### Java

* [Oracle JDK](https://www.oracle.com/technetwork/java/javase/downloads/index.html)
* [Stack Overflow article on downloading Oracle JDK with wget](https://stackoverflow.com/questions/10268583/downloading-java-jdk-on-linux-via-wget-is-shown-license-page-instead)

##### Ruby

* [Ruby](https://www.ruby-lang.org/en/downloads/)
* [RVM](https://rvm.io)

##### PHP

* [PHP](https://secure.php.net/downloads.php)
* [phpbrew](https://github.com/phpbrew/phpbrew)

##### Golang
* [Golang](https://golang.org/dl/)
* [gvm](https://github.com/moovweb/gvm)


#### Docker
* [Installing Docker on Fedora](https://docs.docker.com/install/linux/docker-ce/fedora/#install-docker-ce)

#### Ansible
* [installed using Python pip](https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html#latest-releases-via-pip)

#### Others
* [Postman](https://www.getpostman.com/apps)
* [pfSense](https://www.pfsense.org/download/)
* [Homebrew](https://brew.sh/)
* [Fedy](https://www.folkswithhats.org/)



